...
fun_expr:
  ...
  | pbop_op = mkrhs(LETOP) bindings = letop_bindings IN body = seq_expr
      { let (pbop_pat, pbop_exp, rev_ands) = bindings in
        ...
        mkexp ~loc:$sloc (Pexp_letop{ let_=let_; ands=ands; body}) }
...
%inline metaocaml_expr:
  | METAOCAML_ESCAPE e = simple_expr
    { mkexp ~loc:$sloc (pexp_extension ~loc:$sloc ~id:(mknoloc "metaocaml.escape") e) }
  | METAOCAML_BRACKET_OPEN e = seq_expr METAOCAML_BRACKET_CLOSE
    { mkexp ~loc:$sloc (pexp_extension ~loc:$sloc ~id:(mknoloc "metaocaml.bracket") e) }
;
