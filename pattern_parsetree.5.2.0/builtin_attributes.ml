...
let mark_alert_used a =
  ...
  then mark_used (loc_map unvala a.attr_name)
...
let alert_attr x =
  if attr_equals_builtin x "deprecated" then
    ...
    | Some (kind, message) -> Some (x, unvala kind, unvala message)
...
let warning_attribute ?(ppwarning = true) =
  ...
  let process_alert loc name = function
    | PStr(Ploc.VaVal [{pstr_desc=
              Pstr_eval(
                Ploc.VaVal {pexp_desc=Pexp_constant(Ploc.VaVal (Pconst_string(s,_,_)))},
                _)
           }]) ->
        begin
          mark_used name;
          try Warnings.parse_alert_option (unvala s)
          with Arg.Bad msg -> warn_payload loc name.txt msg
        end
    | k ->
  ...
  fun ({attr_name; attr_loc; attr_payload} as attr) ->
    if attr_equals_builtin attr "warning" then
      process attr_loc (loc_map unvala attr_name) false attr_payload
    else if attr_equals_builtin attr "warnerror" then
      process attr_loc (loc_map unvala attr_name) true attr_payload
    else if attr_equals_builtin attr "alert" then
      process_alert attr_loc (loc_map unvala attr_name) attr_payload
    else if ppwarning && attr_equals_builtin attr "ppwarning" then
      begin match attr_payload with
      | PStr (Ploc.VaVal [{ pstr_desc=
                  Pstr_eval(Ploc.VaVal {pexp_desc=Pexp_constant
                                         (Ploc.VaVal (Pconst_string (s, _, _)))},_);
                pstr_loc }]) ->
        (mark_used (loc_map  unvala attr_name);
         Location.prerr_warning pstr_loc (Warnings.Preprocessor (unvala s)))
      | _ ->
        (mark_used (loc_map unvala attr_name);
         warn_payload attr_loc (unvala attr_name.txt)
...
let has_attribute nm attrs =
  List.exists
    (fun a ->
       ...
       then (mark_used (loc_map unvala a.attr_name); true)
       else false)
    attrs
