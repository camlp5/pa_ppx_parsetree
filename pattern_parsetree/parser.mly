...
labeled_simple_pattern:
    QUESTION LPAREN label_let_pattern opt_default RPAREN
      { (Optional (vaval (fst $3)), $4, snd $3) }
  | QUESTION label_var
      { (Optional (vaval (fst $2)), None, snd $2) }
  | OPTLABEL LPAREN let_pattern opt_default RPAREN
      { (Optional (vaval $1), $4, $3) }
  | OPTLABEL pattern_var
      { (Optional (vaval $1), None, $2) }
...
;
%inline label_var:
    mkrhs(vala(LIDENT, ANTI_LID))
      { (unvala $1.Location.txt, mkpat ~loc:$sloc (Ppat_var $1)) }
;
let_pattern:
    pattern
      { $1 }
...
expr:
  ...
  | indexop_expr_1(DOT, seq_expr, LESSMINUS v=expr {Some v})
    { mk_indexop_expr builtin_indexing_operators ~loc:$sloc $1 }
  | indexop_expr_2(qualified_dotop, expr_semi_list, LESSMINUS v=expr {Some v})
