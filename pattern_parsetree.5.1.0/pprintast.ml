...
and pattern1 ctxt (f:Format.formatter) (x:pattern) : unit =
  ...
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_construct (({txt=Ploc.VaVal (Lident(Ploc.VaVal ("()"|"[]")));_}), _) ->
...
and simple_pattern ctxt (f:Format.formatter) (x:pattern) : unit =
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_construct (({txt=Ploc.VaVal (Lident (Ploc.VaVal ("()"|"[]" as x)));_}), Ploc.VaVal None) ->
        pp f  "%s" x
    | Ppat_any -> pp f "_";
    ...
    | Ppat_open (lid, p) ->
        ...
        | Ppat_construct (({txt=Ploc.VaVal (Lident (Ploc.VaVal ("()"|"[]")));_}), Ploc.VaVal None) -> false
        | _ -> true in
...
and binding ctxt f {pvb_pat=p; pvb_expr=x; pvb_constraint = ct; _} =
  ...
  | Some (Pvc_constraint { locally_abstract_univars = Ploc.VaVal vars; typ }) ->
    ...
    (simple_pattern ctxt) p (list pp_print_string ~sep:"@;")
        (List.map (function x -> unvala x.txt) vars)
