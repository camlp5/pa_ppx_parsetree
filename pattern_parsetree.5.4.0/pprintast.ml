...
(* Turn an arbitrary variable name into a valid OCaml identifier by adding \#
  ...
  let rec any_longident ~kind f = function
    | Lident s -> ident_of_name ~kind f (unvala s)
    | Ldot(y,s) ->
        protect_longident ~kind f (any_longident ~kind:Other) (unvala y.txt) (unvala s.txt)
    | Lapply (y,s) ->
        Format_doc.fprintf f "%a(%a)"
          (any_longident ~kind:Other) (unvala y.txt)
          (any_longident ~kind:Other) (unvala s.txt)
...
let constr_vala ppf l = constr ppf (unvala l)
      ...
      let rec loop exp acc = match exp with
          ...
          | {pexp_desc=
             Pexp_construct ({txt=Ploc.VaVal (Lident (Ploc.VaVal "::"));_},
                             Ploc.VaVal (Some ({pexp_desc= Pexp_tuple(Ploc.VaVal [Ploc.VaVal None, e1; Ploc.VaVal None, e2]);
                                    pexp_attributes = Ploc.VaVal []})));
             pexp_attributes = Ploc.VaVal []}
            ->
              loop e2 (e1::acc)
...
and tuple_type_component ctxt f (label, ty) =
  begin match unvala label with
  | None   -> ()
  | Some s -> pp f "%s:" (unvala s)
  end;
...
and core_type1 ctxt f x =
  if unvala x.ptyp_attributes <> [] then core_type ctxt f x
    ...
    | Ptyp_tuple l ->
        pp f "(%a)" (list (tuple_type_component ctxt) ~sep:"@;*@;") (unvala l)
    | Ptyp_constr (li, Ploc.VaVal l) ->
        pp f (* "%a%a@;" *) "%a%a"
          ...
          l (with_loc type_longident_vala) li
    ...
    | Ptyp_open(li, ct) ->
       pp f "@[<hov2>%a.(%a)@]" value_longident_loc (loc_map unvala li) (core_type ctxt) ct
...
and package_type ctxt f ptyp =
  let aux f (s, ct) =
    pp f "type %a@ =@ %a" (with_loc type_longident_vala) s (core_type ctxt) ct
  in
  match unvala ptyp.ppt_cstrs with
  | [] -> with_loc type_longident_vala f ptyp.ppt_path
  | _ ->
    pp f "%a@ with@ %a"
      (with_loc type_longident_vala) ptyp.ppt_path
      (list aux ~sep:"@ and@ ") (unvala ptyp.ppt_cstrs)
...
and pattern1 ctxt (f:Format.formatter) (x:pattern) : unit =
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_construct ({txt=Ploc.VaVal (Lident (Ploc.VaVal "::"));_}, Ploc.VaVal (Some (Ploc.VaVal [],
        {ppat_desc = Ppat_tuple(Ploc.VaVal [Ploc.VaVal None, pat1; Ploc.VaVal None, pat2], Ploc.VaVal Closed);_}))) ->
        (* Right associative*)
    ...
    | Ppat_construct (li, po) ->
        ...
        (match unvala po with
          | Some (Ploc.VaVal [], x) ->
              ...
              pp f "%a@;%a"  value_longident_vala_loc li (simple_pattern ctxt) x
...
and tuple_pattern_component ctxt (f:Format.formatter) (label, x) : unit =
  let simple_name = match x with
    | {ppat_desc = Ppat_var { txt=s; _ }; ppat_attributes = Ploc.VaVal []; _} -> Some s
    | _ -> None
  in
  match unvala label, simple_name with
  (* Labeled component can be represented with pun *)
  | Some lbl, Some simple_name when String.equal (unvala simple_name) (unvala lbl) ->
    pp f "~%s" (unvala lbl)
  (* Labeled component general case *)
  | Some lbl, _ -> pp f "~%s:%a" (unvala lbl) (pattern1 ctxt) x
  (* Unlabeled component *)
...
and simple_pattern ctxt (f:Format.formatter) (x:pattern) : unit =
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_type li ->
        pp f "#%a" (with_loc type_longident_vala) li
    | Ppat_record (l, closed) ->
        let longident_x_pattern f (li, p) =
          ...
          | ({txt=Ploc.VaVal (Lident s);_ },
            ...
            when s = txt ->
              pp f "@[<2>%a@]"  value_longident_vala_loc li
          | _ ->
              pp f "@[<2>%a@;=@;%a@]" value_longident_vala_loc li (pattern1 ctxt) p
        in
    ...
    | Ppat_tuple (Ploc.VaVal l,Ploc.VaVal c) -> tuple_pattern ctxt f l c
    ...
    | Ppat_open (lid, p) ->
        ...
        | Ppat_construct (({txt=Ploc.VaVal (Lident (Ploc.VaVal ("()"|"[]"|"true"|"false")));_}), Ploc.VaVal None) ->
            false
        | _ -> true in
        pp f "@[<2>%a.%a @]" value_longident_vala_loc lid
...
and sugar_expr ctxt f e =
  ...
  | Pexp_apply ({ pexp_desc = Pexp_ident {txt = id; _};
    ...
    when List.for_all (fun (lab, _) -> lab = Nolabel) (unvala args) -> begin
      ...
      | Ldot ({txt=path;_}, {txt=Ploc.VaVal ("get"|"set" as func);_}), a :: other_args ->
        begin
          ...
          | Lident (Ploc.VaVal "String"), i :: rest ->
            print ".[" "" "]" (expression ctxt) [i] rest
          | Ldot ({txt=Ploc.VaVal (Lident (Ploc.VaVal "Bigarray"));_}, {txt=Ploc.VaVal "Array1";_}), i1 :: rest ->
            print ".{" "," "}" (simple_expr ctxt) [i1] rest
          | Ldot ({txt=Ploc.VaVal (Lident (Ploc.VaVal "Bigarray"));_}, {txt=Ploc.VaVal "Array2";_}),
                  i1 :: i2 :: rest ->
            print ".{" "," "}" (simple_expr ctxt) [i1; i2] rest
          | Ldot ({txt=Ploc.VaVal (Lident (Ploc.VaVal "Bigarray"));_}, {txt=Ploc.VaVal "Array3";_}),
                  i1 :: i2 :: i3 :: rest ->
            print ".{" "," "}" (simple_expr ctxt) [i1; i2; i3] rest
          | Ldot ({txt=Ploc.VaVal (Lident (Ploc.VaVal "Bigarray"));_}, {txt=Ploc.VaVal "Genarray";_}),
        ...
        when first_is '.' (unvala s) ->
          ...
          *)
          ...
          let path_prefix = match unvala id with
            | Ldot(m,_) -> Some m.txt
            | _ -> None in
          let left = String.sub (unvala s) 0 (1+String.index (unvala s) left) in
...
and expression ctxt f x =
  if unvala x.pexp_attributes <> [] then
    ...
    | Pexp_construct (li, Ploc.VaVal (Some eo))
      when not (is_simple_construct (view_expr x))-> (* Not efficient FIXME*)
        (match view_expr x with
         ...
         | `normal ->
             pp f "@[<2>%a@;%a@]" (with_loc constr_vala) li
               (simple_expr ctxt) eo
         | _ -> assert false)
    | Pexp_setfield (e1, li, e2) ->
        pp f "@[<2>%a.%a@ <-@ %a@]"
          (simple_expr ctxt) e1 value_longident_vala_loc li (simple_expr ctxt) e2
    ...
    | Pexp_new (li) ->
        pp f "@[<hov2>new@ %a@]" (with_loc type_longident_vala) li;
    | Pexp_setinstvar (s, e) ->
...
and expression2 ctxt f x =
  if unvala x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_field (e, li) ->
        pp f "@[<hov2>%a.%a@]" (simple_expr ctxt) e value_longident_vala_loc li
    | Pexp_send (e, s) ->
  ...
  if unvala x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_construct _  when is_simple_construct (view_expr x) ->
        (match view_expr x with
         ...
         | `simple x -> constr_vala f x
         | _ -> assert false)
    | Pexp_ident li ->
        value_longident_vala_loc f li
    ...
    | Pexp_pack (me, opty) ->
        ...
        pp f ")"
    | Pexp_tuple l ->
        pp f "@[<hov2>(%a)@]" (list (tuple_expr_component ctxt) ~sep:",@;") (unvala l)
    ...
    | Pexp_record (l, eo) ->
        let longident_vala_x_expression f ( li, e) =
          match e with
          |  {pexp_desc=Pexp_ident {txt;_};
              pexp_attributes=Ploc.VaVal []; _} when Longident.same (unvala li.txt) (unvala txt) ->
              pp f "@[<hov2>%a@]" value_longident_vala_loc li
          | _ ->
              pp f "@[<hov2>%a@;=@;%a@]"
                value_longident_vala_loc li
                (simple_expr ctxt) e
        in
        pp f "@[<hv0>@[<hv2>{@;%a%a@]@;}@]"(* "@[<hov2>{%a%a}@]" *)
          (option ~last:" with@;" (simple_expr ctxt)) (unvala eo)
          (list longident_vala_x_expression ~sep:";@;") (unvala l)
    | Pexp_array (l) ->
        pp f "@[<0>@[<2>[|%a|]@]@]"
...
and class_type ctxt f x =
  ...
  | Pcty_open (o, e) ->
      pp f "@[<2>let open%s %a in@;%a@]"
        (override_vala o.popen_override) value_longident_vala_loc o.popen_expr
        (class_type ctxt) e
...
and module_type1 ctxt f x =
  if unvala x.pmty_attributes <> [] then module_type ctxt f x
    ...
    | Pmty_ident li ->
        pp f "%a" (with_loc type_longident_vala) li;
    | Pmty_alias li ->
        pp f "(module %a)" (with_loc type_longident_vala) li;
    | Pmty_signature (s) ->
...
and signature_item ctxt f x : unit =
  ...
  | Psig_module ({pmd_type={pmty_desc=Pmty_alias alias;
      ...
      pp f "@[<hov>module@ %s@ =@ %a@]%a"
        ...
        value_longident_vala_loc alias
  ...
  | Psig_modsubst pms ->
      pp f "@[<hov>module@ %s@ :=@ %a@]%a" (unvala pms.pms_name.txt)
        value_longident_vala_loc pms.pms_manifest
        (item_attributes ctxt) pms.pms_attributes
  | Psig_open od ->
      pp f "@[<hov2>open%s@ %a@]%a"
        (override_vala od.popen_override)
        value_longident_vala_loc od.popen_expr
  ...
  if unvala x.pmod_attributes <> [] then
    ...
    | Pmod_constraint (me, mt) ->
        pp f "@[<hov2>(%a@ :@ %a)@]"
          ...
          (module_type ctxt) mt
    | Pmod_ident (li) ->
        pp f "%a" value_longident_vala_loc li;
...
and type_extension ctxt f x =
  ...
  pp f "@[<2>type %a%a += %a@ %a@]%a"
    ...
    (with_loc type_longident_vala) x.ptyext_path
...
and tuple_expr_component ctxt f (l,e) =
  let simple_name = match e with
    | {pexp_desc=Pexp_ident {txt=Ploc.VaVal (Lident (Ploc.VaVal l));_}; pexp_attributes=Ploc.VaVal []} -> Some l
    | _ -> None
  in match (simple_name, unvala l) with
  (* Labeled component can be represented with pun *)
  | Some simple_name, Some lbl when String.equal simple_name (unvala lbl) ->
    pp f "~%s" (unvala lbl)
  (* Labeled component general case *)
  | _, Some lbl -> pp f "~%s:%a" (unvala lbl) (simple_expr ctxt) e
  (* Unlabeled component *)
