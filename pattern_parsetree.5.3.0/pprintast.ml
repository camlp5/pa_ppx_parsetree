...
(* Turn an arbitrary variable name into a valid OCaml identifier by adding \#
  ...
  let rec any_longident ~kind f = function
    ...
    | Ldot(y,s) ->
        protect_longident ~kind f (any_longident ~kind:Other) (unvala y) (unvala s)
    | Lapply (y,s) ->
...
and core_type1 ctxt f x =
  if unvala x.ptyp_attributes <> [] then core_type ctxt f x
    ...
    | Ptyp_package (lid, cstrs) ->
        let aux f (s, ct) =
          pp f "type %a@ =@ %a"
            (with_loc type_longident_vala) s
            (core_type ctxt) ct  in
        (match unvala cstrs with
         |[] -> pp f "@[<hov2>(module@ %a)@]" (with_loc type_longident_vala) lid
         |_ ->
             pp f "@[<hov2>(module@ %a@ with@ %a)@]"
               (with_loc type_longident_vala) lid
               (list aux  ~sep:"@ and@ ")  (unvala cstrs))
    | Ptyp_open(li, ct) ->
       pp f "@[<hov2>%a.(%a)@]" value_longident_vala_loc li (core_type ctxt) ct
