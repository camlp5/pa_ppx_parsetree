...
let uncapitalize s =  Ok(String.uncapitalize_ascii s)
let capitalize s =  Ok(String.capitalize_ascii s)
...
let is_lowercase s = s = String.lowercase_ascii s

let is_valid_identifier s = assert false
let validate_identifier ?with_dot s = Valid
let starts_like_a_valid_identifier s = true
end
# 518 "misc.ML"
(* File functions *)
