...
module Const = struct
  ...
  let string ?quotation_delimiter s = Pconst_string (vaval s, Option.map vaval quotation_delimiter)
end
...
module Type = struct
  ...
  let constructor ?(loc = !default_loc) ?(attrs = Ploc.VaVal []) ?(info = empty_info)
        ?(args = Pcstr_tuple (Ploc.VaVal [])) ~res name =
    {
     pcd_name = name;
     pcd_args = args;
     pcd_res = res;
...
module Te = struct
  ...
  let decl ?(loc = !default_loc) ?(attrs = Ploc.VaVal []) ?(docs = empty_docs)
         ?(info = empty_info) ?(args = Pcstr_tuple (Ploc.VaVal [])) ~res name =
    {
     pext_name = name;
