...
let fmt_constant f x =
  ...
  | Pconst_string (Ploc.VaVal s, None) -> fprintf f "PConst_string(%S,None)" s;
  | Pconst_string (Ploc.VaVal s, Some (Ploc.VaVal delim)) ->
...
let rec core_type i ppf x =
  ...
  | Ptyp_poly (sl, ct) ->
      line i ppf "Ptyp_poly%a\n"
        (fun ppf ->
           List.iter (fun x -> fprintf ppf " %a" Pprintast.tyvar (unvala x.txt))
        )
        (unvala sl);
      core_type i ppf ct;
  | Ptyp_package (s, l) ->
