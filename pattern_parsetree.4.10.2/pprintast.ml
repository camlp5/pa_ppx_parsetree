...
let constant f = function
  | Pconst_char (Ploc.VaVal i) ->
      pp f "%C"  i
  | Pconst_string (Ploc.VaVal i, None) ->
      pp f "%S" i
  | Pconst_string (Ploc.VaVal i, Some (Ploc.VaVal delim)) ->
...
and pattern ctxt f x =
  ...
  if unvala x.ppat_attributes <> [] then begin
    ...
    | Ppat_alias (p, s) ->
        pp f "@[<2>%a@;as@;%a@]" (pattern ctxt) p protect_ident (unvala s.txt) (* RA*)
    | Ppat_or _ -> (* *)
...
and pattern1 ctxt (f:Format.formatter) (x:pattern) : unit =
  ...
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_construct (({txt=Ploc.VaVal (Lident(Ploc.VaVal ("()"|"[]")));_}), _) -> simple_pattern ctxt f x
    | Ppat_construct (({txt;_} as li), po) ->
        ...
        if unvala txt = Lident (Ploc.VaVal "::") then
          ...
          (match unvala po with
           | Some x -> pp f "%a@;%a"  longident_vala_loc li (simple_pattern ctxt) x
...
and simple_pattern ctxt (f:Format.formatter) (x:pattern) : unit =
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_open (lid, p) ->
        ...
        | Ppat_construct (({txt=Ploc.VaVal (Lident (Ploc.VaVal ("()"|"[]")));_}), _) -> false
        | _ -> true in
...
and expression2 ctxt f x =
  if unvala x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_send (e, s) -> pp f "@[<hov2>%a#%s@]" (simple_expr ctxt) e (unvala s.txt)
  ...
  if unvala x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_coerce (e, cto1, ct) ->
        pp f "(%a%a :> %a)" (expression ctxt) e
          ...
          (core_type ctxt) ct
    | Pexp_variant (l, Ploc.VaVal None) -> pp f "`%s" (unvala l)
    | Pexp_record (l, eo) ->
...
and module_type ctxt f x =
  ...
  | Pwith_modsubst (li, li2) ->
             pp f "module %a :=@ %a" longident_vala_loc li longident_vala_loc li2 in
        pp f "@[<hov2>%a@ with@ %a@]"
          (module_type1 ctxt) mt (list with_constraint ~sep:"@ and@ ") (unvala l)
...
and signature_item ctxt f x : unit =
  ...
  | Psig_typesubst l ->
      type_def_list ctxt f (vaval Nonrecursive, false, unvala l)
  | Psig_value vd ->
...
and binding ctxt f {pvb_pat=p; pvb_expr=x; _} =
  ...
  | Some (p, Ploc.VaVal tyvars, ct, e) -> begin
    pp f "%a@;: type@;%a.@;%a@;=@;%a"
...
and binding_op ctxt f x =
     pp f "@[<2>%s %a@;=@;%a@]"
    (unvala x.pbop_op.txt) (pattern ctxt) x.pbop_pat (expression ctxt) x.pbop_exp
