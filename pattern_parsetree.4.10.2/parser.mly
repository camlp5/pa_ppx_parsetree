  ...
  match bigarray_untuplify arg with
    [c1] ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Array1" get)),
                       vaval [Nolabel, arr; Nolabel, c1]))
  | [c1;c2] ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Array2" get)),
                       vaval [Nolabel, arr; Nolabel, c1; Nolabel, c2]))
  | [c1;c2;c3] ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Array3" get)),
                       vaval [Nolabel, arr; Nolabel, c1; Nolabel, c2; Nolabel, c3]))
  | coords ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Genarray" "get")),
                       vaval [Nolabel, arr; Nolabel, ghexp(Pexp_array (vaval coords))]))
...
let bigarray_set ~loc arr arg newval =
  ...
  match bigarray_untuplify arg with
    [c1] ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Array1" set)),
                       vaval [Nolabel, arr; Nolabel, c1; Nolabel, newval]))
  | [c1;c2] ->
  ...
  | coords ->
      mkexp(Pexp_apply(ghexp(Pexp_ident(bigarray_function "Genarray" "set")),
                       vaval [Nolabel, arr;
                        Nolabel, ghexp(Pexp_array (vaval coords));
                        Nolabel, newval]))
...
let text_def pos = [Ptop_def (vaval (Str.text (rhs_text pos)))]
...
let extra_def p1 p2 items =
  extra_text p1 p2 (fun txt -> [Ptop_def (vaval (Str.text txt))]) items
...
/*-*/%type <(core_type * variance) list> type_parameters
...
/*-*/  | xtr_antis { Pexp_xtr (Location.mkloc $1 (make_loc $sloc)) }
  ...
  | od=open_dot_declaration DOT mkrhs(LPAREN RPAREN {vaval (Lident (vaval "()"))})
      { (* TODO: review the location of Pexp_construct *)
        Pexp_open(od, mkexp ~loc:$sloc (Pexp_construct($3, vaval None))) }
  ...
  | od=open_dot_declaration DOT LBRACKETBAR expr_semi_list BARRBRACKET
      { (* TODO: review the location of Pexp_array *)
        Pexp_open(od, mkexp ~loc:$sloc (Pexp_array(vaval $4))) }
  | od=open_dot_declaration DOT LBRACKETBAR BARRBRACKET
      { (* TODO: review the location of Pexp_array *)
        Pexp_open(od, mkexp ~loc:$sloc (Pexp_array (vaval []))) }
  | mod_longident DOT
    LBRACKETBAR expr_semi_list error
      { unclosed "[|" $loc($3) "|]" $loc($5) }
...
letop_bindings:
    body = letop_binding_body
      { let let_pat, let_exp = body in
        let_pat, let_exp, [] }
  | bindings = letop_bindings pbop_op = mkrhs(vaval(ANDOP)) body = let_binding_body
...
| EXCEPTION
  ...
  { let loc = make_loc $sloc in
    ...
    Te.mk_exception ~attrs
      (vaval (Te.rebind id lid ~attrs:( append_list_vala attrs1 attrs2) ~loc ~docs))
    , ext }
;
sig_exception_declaration:
  EXCEPTION
  ...
  attrs = post_item_attributes_vala
    { let args, res = args_res in
      ...
      let docs = symbol_docs $sloc in
      Te.mk_exception ~attrs
        (vaval (Te.decl id ~args ~res ~attrs:(append_list_vala attrs1 attrs2) ~loc ~docs))
      , ext }
| EXCEPTION
  ext = ext
  ANTI_EXCON
...
%inline extension_constructor_declaration_vala(opening):
  d = generic_constructor_declaration_vala(opening,ANTI_EXCON)
    {
      Pcaml.vala_map (fun (cid, args, res, attrs, loc, info) ->
          Te.decl cid ~args ~res ~attrs ~loc ~info) d
    }
;
extension_constructor_rebind(opening):
  opening
...
constant:
  ...
  | STRING       { let (s, d) = $1 in Pconst_string (vaval s, Option.map vaval d) }
...
/*-*/  | ANTI_STRING     { Pconst_string (vaant $1, None) }
/*-*/  | ANTI_STRING ANTI_DELIM     { Pconst_string (vaant $1, Some (vaant $2)) }
...
val_longident:
    val_ident_vala                                   { Lident $1 }
  | mod_longident_vala DOT val_ident_vala                 { Ldot($1, $3) }
;
...
constr_longident:
  ...
  | mod_longident DOT LPAREN COLONCOLON RPAREN  { Ldot(vaval $1,vaval "::") }
  | LBRACKET RBRACKET                           { Lident (vaval "[]") }
  | LPAREN RPAREN                               { Lident (vaval "()") }
  | LPAREN COLONCOLON RPAREN                    { Lident (vaval "::") }
  | FALSE                                       { Lident (vaval "false") }
  | TRUE                                        { Lident (vaval "true") }
;
...
label_longident:
    lident_vala                                      { Lident $1 }
  | mod_longident_vala DOT lident_vala                    { Ldot($1, $3) }
;
type_longident:
    lident_vala                                      { Lident $1 }
  | mod_ext_longident_vala DOT lident_vala                { Ldot($1, $3) }
;
mod_longident:
    uident_vala                                      { Lident $1 }
  | mod_longident_vala DOT uident_vala                    { Ldot($1, $3) }
;
...
mod_ext_longident:
    uident_vala                                      { Lident $1 }
  | mod_ext_longident_vala DOT uident_vala                { Ldot($1, $3) }
...
mty_longident:
    ident_vala                                       { Lident $1 }
  | mod_ext_longident_vala DOT ident_vala                 { Ldot($1, $3) }
;
clty_longident:
    lident_vala                                      { Lident $1 }
  | mod_ext_longident_vala DOT lident_vala                { Ldot($1, $3) }
