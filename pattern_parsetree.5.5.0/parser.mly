...
let mkctf ~loc ?attrs d =
  Ctf.mk ~loc:(make_loc loc) ?attrs d
let mkcf ~loc ?attrs d =
  Cf.mk ~loc:(make_loc loc) ?attrs d
...
let rec mktailexp nilloc = let open Location in function
  ...
  | e1 :: el ->
      ...
      let arg =
        ghexp ~loc (Pexp_tuple (vaval [vaval None, e1; vaval None, ghexp ~loc:el_loc exp_el]))
      in
...
let rec mktailpat nilloc = let open Location in function
  ...
  | p1 :: pl ->
      ...
      let arg =
        ghpat ~loc
          (Ppat_tuple (vaval [vaval None, p1; vaval None, ghpat ~loc:el_loc pat_pl], vaval Closed))
      in
...
let mkexp_desc_constraint e t =
  ...
  | Pcoerce(t1, t2)  -> Pexp_coerce(e, vaval t1, t2)
...
let bigarray_untuplify exp =
  ...
  | Pexp_tuple (Ploc.VaVal explist)
       when List.for_all (fun (l, _) -> Option.is_none (unvala l)) explist ->
  ...
  let prefix = match paren_kind with
    ...
    | Brace ->
       let submodule_name = match n with
         ...
         | Three -> "Array3"
         | Many -> "Genarray" in
       Ldot(mknoloc (vaval (Lident (vaval "Bigarray"))), mknoloc (vaval submodule_name)) in
   ghloc ~loc (Ldot(mknoloc (vaval prefix), mknoloc (vaval opname)))
...
let user_indexing_operator_name loc (prefix,ext) ~assign paren_kind n =
  ...
  let lid = match Option.map unvala prefix with
    ...
    | Some p -> Ldot(mknoloc (vaval p),mknoloc (vaval name)) in
  ghloc ~loc lid
...
let loc_lident (id : string Location.loc) : Longident.t Location.loc =
  Location.map (fun x -> Lident (vaval x)) id

let exp_of_longident lid =
  let lid = Location.map (fun id -> vaval (Lident (vaval (Longident.last id)))) lid in
...
let pexp_extension ~loc ~id e = Pexp_extension (id, PStr (vaval [mkstrexp ~loc e (vaval [])]))
...
let mkexp_attrs ~loc desc (ext, attrs) =
  ...
  | Some id ->
     mkexp ~loc (pexp_extension ~loc ~id (vaval (ghexp ~loc ~attrs desc)))
...
let mkpat_attrs ~loc desc (ext, attrs) =
  ...
  | Some id ->
     mkpat ~loc (Ppat_extension (id, PPat (ghpat ~loc ~attrs desc, vaval None)))
...
let wrap_mkstr_ext ~loc (item, ext) =
  match ext with
  | None -> mkstr ~loc item
  | Some id -> mkstr ~loc (Pstr_extension ((id, PStr (vaval [ghstr ~loc item])), vaval []))

let wrap_mksig_ext ~loc (item, ext) =
  match ext with
  | None -> mksig ~loc item
  | Some id -> mksig ~loc (Psig_extension ((id, PSig (vaval [ghsig ~loc item])), vaval []))
...
let mkghost_newtype_function_body newtypes body_constraint body =
  let wrapped_body =
    match unvala body_constraint with
    | None -> body
    | Some body_constraint ->
        let loc = { body.pexp_loc with loc_ghost = true } in
        Exp.mk (mkexp_desc_constraint body (unvala body_constraint)) ~loc
  in
  let expr =
    List.fold_right
...
let package_type_of_module_type pmty =
  ...
  let map_cstr = function
    | Pwith_type (lid, Ploc.VaVal ptyp) ->
        ...
        if ptyp.ptype_constraints <> vaval [] then
...
/*-*/%type <Asttypes.arg_label Ploc.vala * Parsetree.expression option Ploc.vala * Parsetree.pattern> simple_param_pattern
...
/*-*/      { mktyp ~loc:$sloc ~attrs (Ptyp_poly(vaant $1, mktyp ~loc:$sloc (Ptyp_xtr (Location.mkloc $3 (make_loc $sloc))))) }
;
...
expr_colon_package_type:
  ...
  | e = expr COLON ty1 = package_type COLONGREATER ty2 = package_type
      { mkexp ~loc:$loc (Pexp_coerce (e, vaval (Some ty1), ty2)) }
  | e = expr COLONGREATER ty2 = package_type
      { mkexp ~loc:$loc (Pexp_coerce (e, vaval None, ty2)) }
;
...
%inline module_binding:
  ...
  attrs2 = post_item_attributes_vala
    { let loc = make_loc $sloc in
      ...
      let body = Mb.mk name body ~attrs ~loc in
      body, ext }
;
...
module_type_declaration:
  ...
  attrs2 = post_item_attributes_vala
  {
    let attrs = append_list_vala attrs1 attrs2 in
    let loc = make_loc $sloc in
    Mtd.mk id ~typ:(vaval typ) ~attrs ~loc, ext
  }
/*-*/| MODULE TYPE
/*-*/  ext = ext
/*-*/  attrs1 = vaval(attributes)
...
(*-*)    let attrs = append_list_vala attrs1 attrs2 in
(*-*)    let loc = make_loc $sloc in
(*-*)    Mtd.mk id ~typ:(vaant typ) ~attrs ~loc, ext
(*-*)  }
;
...
open_declaration:
  ...
  attrs2 = post_item_attributes_vala
  {
    let attrs = append_list_vala attrs1 attrs2 in
    let loc = make_loc $sloc in
    Opn.mk me ~override ~attrs ~loc, ext
  }
;
...
open_description:
  ...
  attrs2 = post_item_attributes_vala
  {
    let attrs = append_list_vala attrs1 attrs2 in
    let loc = make_loc $sloc in
    Opn.mk id ~override ~attrs ~loc, ext
  }
;
...
module_type_subst:
  ...
  attrs2 = post_item_attributes_vala
  {
    let attrs = append_list_vala attrs1 attrs2 in
    let loc = make_loc $sloc in
    Mtd.mk id ~typ:(vaval (Some typ)) ~attrs ~loc, ext
  }
...
%inline class_declaration:
  ...
  attrs2 = post_item_attributes_vala
  {
    ...
    let loc = make_loc $sloc in
    ext,
    Ci.mk id body ~virt ~params ~attrs ~loc
  }
;
...
class_field:
  | INHERIT override_flag_vala vaval(attributes) class_expr
    ...
    post_item_attributes_vala
      { mkcf ~loc:$sloc (Pcf_inherit ($2, $4, self)) ~attrs:(append_list_vala $3 $6) }
  | VAL value post_item_attributes_vala
      { let v, attrs = $2 in
        mkcf ~loc:$sloc (Pcf_val v) ~attrs:(append_list_vala attrs $3) }
  | METHOD method_ post_item_attributes_vala
      { let meth, attrs = $2 in
        mkcf ~loc:$sloc (Pcf_method meth) ~attrs:(append_list_vala attrs $3) }
  | CONSTRAINT vaval(attributes) constrain_field post_item_attributes_vala
      { mkcf ~loc:$sloc (Pcf_constraint $3) ~attrs:(append_list_vala $2 $4) }
  | INITIALIZER vaval(attributes) seq_expr post_item_attributes_vala
      { mkcf ~loc:$sloc (Pcf_initializer $3) ~attrs:(append_list_vala $2 $4) }
  | item_extension post_item_attributes_vala
      { mkcf ~loc:$sloc (Pcf_extension $1) ~attrs:$2 }
...
method_:
    ...
    label = mkrhs(label_vala) COLON ty = possibly_poly_type
...
class_sig_field:
    INHERIT vaval(attributes) class_signature post_item_attributes_vala
      { mkctf ~loc:$sloc (Pctf_inherit $3) ~attrs:(append_list_vala $2 $4) }
  | VAL vaval(attributes) value_type post_item_attributes_vala
      { mkctf ~loc:$sloc (Pctf_val $3) ~attrs:(append_list_vala $2 $4) }
  | METHOD vaval(attributes) private_virtual_flags mkrhs(label_vala)
...
/*-*/      { mkctf ~loc:$sloc (Pctf_method ($5, vaant p, vaant v, $7)) ~attrs:(append_list_vala $2 $8) }
  | CONSTRAINT vaval(attributes) constrain_field post_item_attributes_vala
      { mkctf ~loc:$sloc (Pctf_constraint $3) ~attrs:(append_list_vala $2 $4) }
  | item_extension post_item_attributes_vala
      { mkctf ~loc:$sloc (Pctf_extension $1) ~attrs:$2 }
  ...
  attrs2 = post_item_attributes_vala
    {
      ...
      let loc = make_loc $sloc in
      ext,
      Ci.mk id cty ~virt ~params ~attrs ~loc
    }
;
  ...
  attrs2 = post_item_attributes_vala
    {
      ...
      let loc = make_loc $sloc in
      ext,
      Ci.mk id csig ~virt ~params ~attrs ~loc
    }
;
...
%inline or_function(EXPR):
  ...
  | FUNCTION ext_attributes vala(match_cases, ANTI_CASES)
      { let loc = make_loc $sloc in
        ...
        *)
        let desc = mkfunction (vaval []) (vaval None) (Pfunction_cases (cases, loc, vaval [])) in
...
fun_seq_expr:
  ...
  | fun_expr SEMI PERCENT attr_id seq_expr
    { mkexp_attrs ~loc:$sloc (Pexp_sequence ($1, $5)) (Some $4, vaval []) }
;
seq_expr:
...
simple_param_pattern:
  ...
  | LABEL LPAREN poly_pattern RPAREN
      { (vaval (Labelled (vaval $1)), vaval None, $3) }
  | LPAREN poly_pattern RPAREN
      { (vaval Nolabel, vaval None, $2) }
...
/*-*/      { mkexp ~loc:$sloc (Pexp_letop{ let_=vaant $1; ands=vaant $2; body}) }
  | fun_expr COLONCOLON expr
      { mkexp_cons ~loc:$sloc $loc($2)
          (ghexp ~loc:$sloc (Pexp_tuple(vaval [vaval None,$1;vaval None,$3]))) }
...
/*-*/      { Pexp_ifthenelse($3, $5, vaant $6), $2 }
  ...
  | WHILE ext_attributes seq_expr do_done_expr
      { Pexp_while($3, $4), $2 }
  | FOR ext_attributes pattern EQUAL seq_expr direction_flag_vala seq_expr
    do_done_expr
...
%inline metaocaml_expr:
  | METAOCAML_ESCAPE e = simple_expr
    { mkexp ~loc:$sloc (pexp_extension ~loc:$sloc ~id:(mknoloc (vaval "metaocaml.escape")) (vaval e)) }
  | METAOCAML_BRACKET_OPEN e = seq_expr METAOCAML_BRACKET_CLOSE
    { mkexp ~loc:$sloc (pexp_extension ~loc:$sloc ~id:(mknoloc (vaval "metaocaml.bracket")) (vaval e)) }
;
...
/*-*/  | xtr_antis { Pexp_xtr (Location.mkloc $1 (make_loc $sloc)) }
  ...
  | od=open_dot_declaration DOT mkrhs(LPAREN RPAREN {vaval (Lident (vaval "()"))})
      { Pexp_struct_item(Str.open_ od,
                         mkexp ~loc:($loc($3)) (Pexp_construct($3, vaval None))) }
  ...
  | od=open_dot_declaration DOT LBRACKETBAR expr_semi_list BARRBRACKET
      { Pexp_struct_item(Str.open_ od,
                         mkexp ~loc:($startpos($3), $endpos) (Pexp_array(vaval $4))) }
  | od=open_dot_declaration DOT LBRACKETBAR BARRBRACKET
      { (* TODO: review the location of Pexp_array *)
        Pexp_struct_item(Str.open_ od,
                         mkexp ~loc:($startpos($3), $endpos) (Pexp_array (vaval []))) }
  | mod_longident DOT
    LBRACKETBAR expr_semi_list error
      { unclosed "[|" $loc($3) "|]" $loc($5) }
...
strict_binding:
    EQUAL seq_expr
      { $2 }
  | vala(fun_params, ANTI_LIST) vaval(type_constraint)? EQUAL fun_body
      { ghexp ~loc:$sloc (mkfunction $1 (vaval $2) $4)
      }
;
fun_body:
  | FUNCTION ext_attributes vala(match_cases, ANTI_CASES)
      { let ext, attrs = $2 in
        ...
        | Some _ ->
          (* function%foo extension nodes interrupt the arity *)
            let cases = Pfunction_cases ($3, make_loc $sloc, vaval []) in
            Pfunction_body
              (mkexp_attrs ~loc:$sloc (mkfunction (vaval []) (vaval None) cases) $2)
      }
  | fun_seq_expr
      { Pfunction_body $1 }
;
...
%inline labeled_tuple_element :
  | expr
     { vaval None, $1 }
  | LABEL simple_expr %prec below_HASH
     { vaval (Some (vaval $1)), $2 }
  | TILDE label = label_vala
     { let loc = $loc(label) in
       vaval (Some label), mkexpvar ~loc label }
  | TILDE LPAREN label = label_vala c = type_constraint RPAREN %prec below_HASH
      { vaval (Some label),
...
| l1 = LABEL x1 = simple_expr
  COMMA
  x2 = labeled_tuple_element
    { [ x2; vaval (Some (vaval l1)), x1 ] }
| TILDE l1 = LIDENT
  COMMA
  x2 = labeled_tuple_element
  { let loc = $loc(l1) in
    [ x2; vaval (Some (vaval l1)), mkexpvar ~loc (vaval l1)] }
| TILDE LPAREN l1 = LIDENT c = type_constraint RPAREN
  COMMA
  x2 = labeled_tuple_element
  { let x1 =
      mkexp_constraint ~loc:($startpos($2), $endpos)
        (mkexpvar ~loc:$loc(l1) (vaval l1)) c
    in
    [ x2; vaval (Some (vaval l1)), x1] }
;
...
%inline pattern_(self):
  | self COLONCOLON pattern
      { mkpat_cons ~loc:$sloc $loc($2)
          (ghpat ~loc:$sloc (Ppat_tuple (vaval [vaval None, $1; vaval None, $3], vaval Closed))) }
  | self attribute
      { Pat.attr $1 $2 }
/*-*/  | self ANTI_ALGATTRS
/*-*/      { Pat.attrs $1 (vaant $2) }
  ...
  | mkpat(
    ...
    | labeled_tuple_pattern(self)
        { $1 }
/*-*/  | ANTI_TUPLELIST
/*-*/      { Ppat_tuple (vaant $1, vaval Closed) }
/*-*/  | ANTI_TUPLELIST COMMA DOTDOT
/*-*/      { Ppat_tuple (vaant $1, vaval Open) }
/*-*/  | ANTI_TUPLELIST ANTI_CLOSEDFLAG
/*-*/      { Ppat_tuple (vaant $1, vaant $2) }
...
%inline labeled_tuple_pat_element(self):
  ...
  | TILDE label = label_vala
      { let loc = $loc(label) in
        vaval (Some label), mkpatvar ~loc label }
  | TILDE LPAREN label = label_vala COLON cty = core_type RPAREN %prec COMMA
      { let lbl_loc = $loc(label) in
        let pat_loc = $startpos($2), $endpos in
        let pat = mkpatvar ~loc:lbl_loc label in
        vaval (Some label), mkpat ~loc:pat_loc (Ppat_constraint(pat, cty)) }
;
...
%inline labeled_tuple_pat_element_noprec(self):
  ...
  | TILDE label = label_vala
      { let loc = $loc(label) in
        vaval (Some label), mkpatvar ~loc label }
  | TILDE LPAREN label = label_vala COLON cty = core_type RPAREN
      { let lbl_loc = $loc(label) in
        let pat_loc = $startpos($2), $endpos in
        let pat = mkpatvar ~loc:lbl_loc label in
        vaval (Some label), mkpat ~loc:pat_loc (Ppat_constraint(pat, cty)) }
...
labeled_tuple_pattern(self):
  | reversed_labeled_tuple_pattern(self)
      { let closed, pat = $1 in
        Ppat_tuple(vaval (List.rev pat), vaval closed) }
;
...
/*-*/  constraints = vala(constraints, ANTI_LIST)
/*-*/  attrs2 = post_item_attributes_vala
/*-*/    {
(*-*)      let (kind, priv, manifest) = kind_priv_manifest in
(*-*)      let attrs = attrs2 in
(*-*)      let loc = make_loc $sloc in
(*-*)      Type.mk id ~params ~constraints ~kind ~priv ~manifest ~attrs ~loc
(*-*)    }
/*-*/;
/*-*/
/*-*/
  ...
  attrs2 = post_item_attributes_vala
    {
      ...
      let loc = make_loc $sloc in
      Type.mk
        id ~params ~constraints ~kind ~priv ~manifest ~attrs ~loc
    }
;
%inline constraints:
  ...
  d = generic_constructor_declaration(opening)
    {
      ...
      Type.constructor cid ~vars ~args ~res ~attrs ~loc
    }
;
str_exception_declaration:
  EXCEPTION
...
/*-*/                                  { ($2,$4,vaant $5) }
  | COLON atomic_type %prec below_HASH
                                  { (vaval [],Pcstr_tuple (vaval []),vaval (Some $2)) }
  | COLON vala(typevar_list, ANTI_LIST) DOT atomic_type %prec below_HASH
                                  { ($2,Pcstr_tuple (vaval []),vaval (Some $4)) }
;
...
%inline type_extension(declaration):
  ...
  attrs2 = post_item_attributes_vala
    { let attrs = append_list_vala attrs1 attrs2 in
      let loc = make_loc $sloc in
      Te.mk tid cs ~params ~priv ~attrs ~loc,
      ext }
;
...
extension_constructor_rebind(opening):
  ...
  lid = mkrhs(constr_longident_vala)
  attrs = attributes_vala
      { Te.rebind cid lid ~attrs ~loc:(make_loc $sloc) }
;
...
function_type:
  ...
  | mktyp(
      ...
      codomain = function_type
        { let ty, ltys = tuple in
          ...
          let domain =
            mktyp ~loc:tuple_loc (Ptyp_tuple (vaval ((vaval None, ty) :: ltys)))
          in
          let domain = extra_rhs_core_type domain ~pos:(snd tuple_loc) in
          Ptyp_arrow(vaval (Labelled (vaval label)), domain, codomain) }
    )
    { $1 }
  | label = lident_vala COLON proper_tuple_type %prec MINUSGREATER
    { let ty, ltys = $3 in
  ...
  | mktyp(
      ...
      MINUSGREATER
      codomain = function_type
        { let ptyp = {ptyp with ppt_attrs = append_list_vala (snd attrs) ptyp.ppt_attrs } in
...
tuple_type:
  ...
  | proper_tuple_type %prec below_WITH
    { let ty, ltys = $1 in
      mktyp ~loc:$sloc (Ptyp_tuple (vaval ((vaval None, ty) :: ltys))) }
/*-*/    | ANTI_TUPLELIST
/*-*/      { mktyp ~loc:$sloc (Ptyp_tuple (vaant $1)) }
;
...
%inline labeled_tuple_typ_element :
  | atomic_type %prec STAR
     { vaval None, $1 }
  | label = label_vala COLON ty = atomic_type %prec STAR
     { vaval (Some label), ty }
;
...
delimited_type_supporting_local_open:
  ...
  | mktyp(
      LBRACKET field = tag_field RBRACKET
        { Ptyp_variant(vaval [ field ], vaval Closed, vaval None) }
    | LBRACKET BAR fields = vala(row_field_list, ANTI_LIST) RBRACKET
        { Ptyp_variant(fields, vaval Closed, vaval None) }
    | LBRACKET field = row_field BAR fields = row_field_list RBRACKET
        { Ptyp_variant(vaval (field :: fields), vaval Closed, vaval None) }
    | LBRACKETGREATER BAR? fields = vala(row_field_list, ANTI_LIST) RBRACKET
        { Ptyp_variant(fields, vaval Open, vaval None) }
    | LBRACKETGREATER RBRACKET
        { Ptyp_variant(vaval [], vaval Open, vaval None) }
    | LBRACKETLESS BAR? fields = vala(row_field_list, ANTI_LIST) RBRACKET
        { Ptyp_variant(fields, vaval Closed, vaval (Some (vaval []))) }
    | LBRACKETLESS BAR? fields = vala(row_field_list, ANTI_LIST)
      GREATER
      tags = vala(name_tag_list, ANTI_LIST)
      RBRACKET
        { Ptyp_variant(fields, vaval Closed, vaval (Some tags)) }
...
%inline field_semi:
  mkrhs(label_vala) COLON poly_type_no_attr vaval(attributes) SEMI vaval(attributes)
    { let attrs = append_list_vala $4 $6 in
      Of.tag ~loc:(make_loc $sloc) ~attrs $1 $3 }
;
...
constant:
  | INT          { let (n, m) = $1 in
                   mkconst ~loc:$sloc (Pconst_integer (vaval n, m)) }
  | CHAR         { mkconst ~loc:$sloc (Pconst_char (vaval $1)) }
...
constr_longident:
  ...
  | mod_longident_vala DOT constr_extra_ident { ldot $1 $loc($1) (vaval $3) $loc($3) }
...
mutable_virtual_flags:
    /* empty */
      { Immutable, vaval Concrete }
  | MUTABLE
      { Mutable, vaval Concrete }
  | VIRTUAL
      { Immutable, vaval Virtual }
  | MUTABLE VIRTUAL
  | VIRTUAL MUTABLE
      { Mutable, vaval Virtual }
;
