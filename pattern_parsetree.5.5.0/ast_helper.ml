...
module Const = struct
  ...
  let float ?loc ?suffix f = mk ?loc (Pconst_float (Ploc.VaVal f, suffix))
  let char ?loc c = mk ?loc (Pconst_char (vaval c))
  let string ?quotation_delimiter ?(loc= !default_loc) s =
    mk ~loc (Pconst_string (vaval s, loc, Option.map vaval quotation_delimiter))
end
  ...
  let varify_constructors var_names t =
    ...
    let rec loop t =
      let desc =
        ...
        | Ptyp_tuple lst ->
            Ptyp_tuple (Pcaml.vala_map (List.map (fun (l, t) -> l, loop t)) lst)
        | Ptyp_constr( { txt = Ploc.VaVal (Longident.Lident s) }, Ploc.VaVal [])
    ...
    and loop_package_type ptyp =
      let cstrs = Pcaml.vala_map (List.map (fun (n,typ) -> (n,loop typ) )) ptyp.ppt_constraints in
      { ptyp with
        ppt_constraints = cstrs }
    in
    loop t

  let package_type ?(loc = !default_loc) ?(attrs = Ploc.VaVal []) p c =
    {ppt_loc = loc;
     ppt_path = p;
     ppt_constraints = c;
     ppt_attrs = attrs}
