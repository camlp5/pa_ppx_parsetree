...
(*-*)let value_longident_vala f x = fprintf f "%a" value_longident (unvala x)
...
(*-*)let longident_vala_loc f x = fprintf f "%a" longident (unvala x.txt)
...
let labelled printer f (label, c) =
  ...
  | Labelled s -> pp f "%a:%a" ident_of_name (unvala s) printer c
  | Optional s -> pp f "?%a:%a" ident_of_name (unvala s) printer c
...
and core_type1 ctxt f x =
  if unvala x.ptyp_attributes <> [] then core_type ctxt f x
    ...
    | Ptyp_open(li, ct) ->
       pp f "@[<hov2>%a.(%a)@]" longident_loc (loc_map unvala li) (core_type ctxt) ct
...
and simple_pattern ctxt (f:Format.formatter) (x:pattern) : unit =
  if unvala x.ppat_attributes <> [] then pattern ctxt f x
    ...
    | Ppat_construct (({txt=Ploc.VaVal (Lident (Ploc.VaVal  ("()"|"[]"|"true"|"false" as x)));_}), Ploc.VaVal None) ->
        pp f  "%s" x
    | Ppat_any -> pp f "_";
    | Ppat_var ({txt = txt;_}) -> ident_of_value_name f (unvala txt)
    | Ppat_array l ->
  ...
  if unvala x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_construct _  when is_simple_construct (view_expr x) ->
        (match view_expr x with
         ...
         | _ -> assert false)
    | Pexp_ident li ->
        with_loc value_longident_vala f li
...
and module_type ctxt f x =
  if unvala x.pmty_attributes <> [] then begin
    pp f "((%a)%a)" (module_type ctxt) {x with pmty_attributes=Ploc.VaVal []}
  ...
  | None -> begin
      ...
      | {ppat_desc=Ppat_var {txt;_}; ppat_attributes=Ploc.VaVal []} ->
          if is_method then
            ...
            pp f "%a@ %a" ident_of_name (unvala txt)  pp_print_pexp_function x
          else
...
and type_declaration ctxt f x =
  ...
  let repr f =
    ...
    | Ptype_open ->
        pp f "%t%t@;.." intro priv
    | Ptype_external name ->
        pp f "@;external %a" constant_string (unvala name)
  in
  let constraints f =
    List.iter
      (fun (ct1,ct2,_) ->
