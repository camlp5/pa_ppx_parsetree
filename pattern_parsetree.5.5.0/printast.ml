...
let rec fmt_longident_aux f x =
  ...
  | Longident.Ldot (y, s) -> fprintf f "%a.%s" fmt_longident_aux (unvala y.txt) (unvala s.txt)
  | Longident.Lapply (y, z) ->
      fprintf f "%a(%a)" fmt_longident_aux (unvala y.txt) fmt_longident_aux (unvala z.txt)
...
let fmt_constant i f x =
  ...
  | Pconst_integer (Ploc.VaVal j,m) -> line i f "PConst_int (%s,%a)\n" j fmt_char_option m
  | Pconst_char (Ploc.VaVal c) -> line i f "PConst_char %02x\n" (Char.code c)
  ...
  | Pconst_float (Ploc.VaVal s,m) -> line i f "PConst_float (%s,%a)\n" s fmt_char_option m
...
let labeled_tuple_element f i ppf (l, ct) =
  option i string ppf (Option.map unvala (unvala l));
  f i ppf ct
...
let rec core_type i ppf x =
  ...
  | Ptyp_open (mod_ident, t) ->
      line i ppf "Ptyp_open \"%a\"\n" fmt_longident_vala_loc mod_ident;
      core_type i ppf t
...
and package_type i ppf ptyp =
  let i = i + 1 in
  line i ppf "package_type %a\n" fmt_longident_vala_loc ptyp.ppt_path;
  list i package_with ppf (unvala ptyp.ppt_constraints);
...
and pattern i ppf x =
  ...
  | Ppat_constant (c) ->
      ...
      fmt_constant i ppf (unvala c);
  | Ppat_interval (c1, c2) ->
      line i ppf "Ppat_interval\n";
      fmt_constant i ppf (unvala c1);
      fmt_constant i ppf (unvala c2);
  | Ppat_tuple (Ploc.VaVal l, Ploc.VaVal c) ->
...
and expression i ppf x =
  ...
  | Pexp_function (params, c, body) ->
      ...
      list i function_param ppf (unvala params);
      option i type_constraint ppf (unvala c);
  ...
  | Pexp_tuple (l) ->
      ...
      list i (labeled_tuple_element expression) ppf (unvala l);
...
and function_param i ppf { pparam_desc = desc; pparam_loc = loc } =
  ...
  | Pparam_val (l, eo, p) ->
      ...
      arg_label (i+1) ppf (unvala l);
      option (i+1) expression ppf (unvala eo);
      pattern (i+1) ppf p
  | Pparam_newtype ty ->
      line i ppf "Pparam_newtype \"%s\" %a\n" (unvala ty.txt) fmt_location loc
...
and type_declaration i ppf x =
  ...
  list (i+1) core_type_x_core_type_x_location ppf (unvala x.ptype_constraints);
...
and type_extension i ppf x =
  ...
  list (i+1) extension_constructor ppf (List.map unvala (unvala x.ptyext_constructors));
...
and type_exception i ppf x =
  ...
  let i = i+1 in
  extension_constructor i ppf (unvala x.ptyexn_constructor)
...
and signature_item i ppf x =
  ...
  | Psig_module pmd ->
      line i ppf "Psig_module %a\n" fmt_str_vala_opt_vala_loc pmd.pmd_name;
...
and module_declaration i ppf pmd =
  str_vala_opt_vala_loc i ppf pmd.pmd_name;
  attributes i ppf pmd.pmd_attributes;
  module_type (i+1) ppf pmd.pmd_type;

and module_binding i ppf x =
  str_vala_opt_vala_loc i ppf x.pmb_name;
...
and value_constraint i ppf x =
  ...
  let pp_newtypes = Format.pp_print_list fmt_string_vala_loc ~pp_sep in
  match x with
  | Pvc_constraint { locally_abstract_univars = Ploc.VaVal []; typ } ->
      core_type i ppf typ
  | Pvc_constraint { locally_abstract_univars=Ploc.VaVal newtypes; typ} ->
